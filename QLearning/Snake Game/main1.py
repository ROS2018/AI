...
class DN(nn.Module):
 def __init__(self, snake_len):
  ...
  self.fc1 = nn.Linear(in_features=snake_len * 2, out_features=snake_len * 4)
  # self.fc22 = nn.Linear(in_features=snake_len * 4, out_features=snake_len * 8)
  # self.fc222 = nn.Linear(in_features=snake_len * 8, out_features=snake_len * 4)
  self.fc2 = nn.Linear(in_features=snake_len * 4, out_features=snake_len * 2)
  self.fc3 = nn.Linear(in_features=snake_len * 2, out_features=snake_len)
  self.fc4 = nn.Linear(in_features=snake_len, out_features=20)
  self.out = nn.Linear(in_features=20, out_features=4)

 def forward(self, t):
  t = t.reshape(-1, self.snake_len * 2)
  t = self.fc1(t)
  t = F.relu(t)
  # t = self.fc22(t)
  # t = F.relu(t)
...
class Agent():
 ...
 def decide(self, policy_net, state):
  ...
  if rate > random.random():  # explore
   ...
   with torch.no_grad():
    a = policy_net(state)#.argmax(dim=1).to(self.device)
...
batch_size = 256
gamma = 0.999  # it is the 0.999 that made me crazy :P
eps_start = 1
eps_end = 0.1
eps_decay = 0.00005
target_update = 5
...
policy_net = DN(em.state_len).to(device)
target_net = DN(em.state_len).to(device)  # target_net =  policy_net
# --
...
for episode in range(num_episodes):  # 4 for each episode
 ...
 for step in count():  # range(20): #count(): # B) for each step:
  ...
  # print('Episode: ', episode, '| Step: ', step) #, ' | exp_size: ', len(memory.memory))
  ...
  if done:
   ...
   dqn_tools.plot(exploration_rate,loss_history,scores, 100)
   break
